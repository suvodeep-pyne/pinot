...
public class RealtimeQuickStart2 {
  private StreamDataServerStartable _kafkaStarter;

  public static void main(String[] args)
      throws Exception {
    PluginManager.get().init();
    new RealtimeQuickStart2().execute();
  }
  ...
  public void execute()
    ...
    File baseDir = new File(quickstartTmpDir, "mockEvent");
    File dataDir = new File(baseDir, "rawdata");
    Preconditions.checkState(dataDir.mkdirs());

    File schemaFile = new File(baseDir, "mockEvent_schema.json");
    File tableConfigFile = new File(baseDir, "mockEvent_realtime_table_config.json");

    ClassLoader classLoader = Quickstart.class.getClassLoader();
    URL resource = classLoader.getResource("examples/stream/mockEvent/schema.json");
    Preconditions.checkNotNull(resource);
    FileUtils.copyURLToFile(resource, schemaFile);
    resource = classLoader.getResource("examples/stream/mockEvent/table_config.json");
    Preconditions.checkNotNull(resource);
