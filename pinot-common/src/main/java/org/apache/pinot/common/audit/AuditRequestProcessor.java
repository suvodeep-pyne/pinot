...
public class AuditRequestProcessor {
  ...
  public AuditEvent processRequest(ContainerRequestContext requestContext, HttpHeaders httpHeaders, String remoteAddr) {
    try {
      ...
      if (_configManager != null && !_configManager.isEnabled()) {
        return null;
      }

      // Check endpoint exclusions (if config manager is available)
      if (_configManager != null && _configManager.isEndpointExcluded(endpoint)) {
        return null;
      }
